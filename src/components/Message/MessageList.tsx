...
const MessageList = () => {
  ...
  return (
    <div className="flex h-full w-[38rem] flex-shrink-0 flex-col items-start gap-[1.7rem] border-r border-solid border-black-20 bg-black-5 p-[2.2rem]">
      ...
      {!isLoading && !isError && conversations.length === 0 && (
        <div className="flex w-full flex-1 flex-col items-center justify-center self-stretch">
          <span className="text-[1.6rem] font-[500] leading-[1.5] tracking-[-0.032rem] text-black-60">
            {debouncedQuery ? '검색 결과가 없습니다.' : '대화 중인 메시지가 없습니다.'}
          </span>
        </div>
      )}
      {!isLoading && !isError && conversations.length > 0 && (
        <div ref={scrollRef} onScroll={handleScroll} className="flex w-full flex-col overflow-y-auto">
          <ul>
